...
function Provider({children}){
    ...
    const link = 'https://localhost:9000';
    const nameRegex = arg => {
        const regex = /^[A-Za-z ']+$/;
        ...
        if(val.split(' ').length > 4){
            arg.target.value = val.split(' ').splice(0,4).join(' ');
        }
    } 

    const numberRegex = arg => {
        const regex = /^[0-9]+$/;
        const val = arg.target.value
        if(!val.match(regex)){  
            arg.target.value = val.split('').filter(a => a.match(regex)).join('')
    ...
    const paswordRegex = arg =>{
        const val = arg.target.value;
        const seriRegex = /^[A-Ca-c]+$/
        if(val.length <3){
            if(!val.match(seriRegex)){
                arg.target.value = val.split('').filter(a => a.match(seriRegex)).join('')
            }
        }
        if(val.length>2){
            const regex = /^[0-9]+$/;
            if(!val.substring(2).match(regex)){  
                arg.target.value = arg.target.value.substring(0, val.length-1)
            } 
        }
        if(val.length>9){
            arg.target.value = arg.target.value.substring(0, 9)
        }
    }

    const [malaka1] = useState([
        { value: 'sport1', label: 'Sport targ‘ibotchisi 2 hafta (72 soat)' },
        { value: 'sport2', label: 'Sport turi bo‘yicha hakam 2 hafta (72 soat)' },
        { value: 'sport3', label: 'Boks sporti bo‘yicha hakam 2 hafta (72 soat)' },
        { value: 'sport4', label: 'Ot sporti bo‘yicha hakam 2 hafta (72 soat)' },
        { value: 'sport4', label: 'Shaxmat 2 hafta (72 soat)' }
      ])

    const [liveaddress] = useState([
    ...
    const [info1] = useState([
        { value: "oliy", label: "Oliy" },
        { value: "orta", label: "O'rta-maxsus/Professional" }
    ])

    const [education]= useState(
        [
            { value:"0", label:"yo'q"},
            { value: "1", label: "Xalqaro toifadagi O'zbekiston sport ustasi yoki xalqaro grossmeyster" },
            { value: "2", label: "O'zbekiston sport ustasi yoki O'zbekiston grossmeysteri" },
            { value: "3", label:"O'zbekiston sport ustaligiga yoki O'zbekiston grossmeysterligiga nomzod"},
            { value: "4", label:"birinchi sport razryadi"},
            { value: "5", label: "ikkinchi sport razryadi" },
            { value: "6", label: "uchinchi sport razryadi" },
          ]
    )

    const [shakl1] = useState([
        { value: "kunduzgi", label:"kunduzgi"},
        { value: "masofaviy", label: "masofaviy" },
    ])

    const [ turi1] = useState([
        { value: "shartnoma", label: "shartnoma asosida" },
        { value: "reja", label: "reja asosida" }
    ])
...
const schema = {
    ...
    return(<>
        <Context.Provider value={{schema, til1, turi1, shakl1, education, info1,district1,liveaddress,malaka1, token, setToken, link, nameRegex,phoneRegex, numberRegex, paswordRegex}}>{children}</Context.Provider>
    </>)
}
